...
export function AppSidebar({
  ...
  const roomsCompleted =
    progressFromApi?.roomsCompleted ?? sideBarData.progress.roomsCompleted;
  const totalRooms =
    progressFromApi?.totalRooms ?? sideBarData.progress.totalRooms;
  const progressPercentage =
    totalRooms > 0 ? (roomsCompleted / totalRooms) * 100 : 0;
  const nextRoomIndex = Math.min(roomsCompleted, ROOMS.length - 1);
  const nextRoom = ROOMS[nextRoomIndex];
  const hasNextRoom = roomsCompleted < totalRooms && nextRoom != null;
        ...
        <div className="px-4 mt-auto mb-4 group-data-[collapsible=icon]:hidden">
          ...
          <div className="rounded-xl bg-background p-4 shadow-sm">
            ...
            <div className="h-2.5 w-full rounded-full bg-muted overflow-hidden">
              <div
                className="h-full bg-gradient-to-r from-[#5a47c7] via-[#9160a8] to-[#dc9b6c] transition-all"
                  ...
                  <div className="flex items-center gap-3 text-left">
                    ...
                    <div>
                      ...
                      <p className="text-xs opacity-90">
                        Room {nextRoom.id} – {nextRoom.subtitle}
                      </p>
                    </div>
                  </div>
